...
interface GameCanvasProps {
  ...
  onPlayerMove: (x: number, y: number, facing: 'left' | 'right', isMoving: boolean, inVent?: boolean, ventId?: string) => void;
...
export function GameCanvas({
  ...
  // Current room indicator computed from player state
  ...
  useEffect(() => {
    ...
    const loop = (time: number) => {
      ...
      if (canvas) {
        ...
        if (ctx) {
          // 1. Process Movement if player is alive, not in task, and not in vent
          if (localPlayer.isAlive && !activeTask && !localPlayer.inVent) {
            let dx = 0;
            let dy = 0;
            ...
            if (isMoving) {
              ...
              const nextY = posRef.current.y + moveY;

              const isCurrentlyStuck = checkCollision(posRef.current.x, posRef.current.y);

              if (!isCurrentlyStuck) {
                if (!checkCollision(nextX, posRef.current.y)) {
                  posRef.current.x = Math.max(60, Math.min(MAP_WIDTH - 60, nextX));
                }
                if (!checkCollision(posRef.current.x, nextY)) {
                  posRef.current.y = Math.max(60, Math.min(MAP_HEIGHT - 60, nextY));
                }
              } else {
                // Fail-safe: if ever stuck inside a collider, allow moving freely to exit
                posRef.current.x = Math.max(60, Math.min(MAP_WIDTH - 60, nextX));
                posRef.current.y = Math.max(60, Math.min(MAP_HEIGHT - 60, nextY));
              }
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-3 sm:top-4 inset-x-3 sm:inset-x-4 flex justify-between items-start pointer-events-none z-30 gap-2">
        ...
        {/* Right: Sound Toggle, Map Toggle & Role Pill */}
        <div className="flex items-center gap-1.5 sm:gap-2.5 pointer-events-auto">
          ...
          {/* Control Scheme Switcher (Mobile/Tablet Joystick) */}
          <button
            type="button"
            onClick={() => {
      ...
      {/* BOTTOM LEFT: Virtual Joystick (Touch / D-Pad on Mobile & Tablet) */}
      ...
      {/* BOTTOM RIGHT: Action Buttons (USE, KILL, REPORT, VENT) */}
      <div className="absolute bottom-4 sm:bottom-6 right-4 sm:right-6 flex items-center gap-2 sm:gap-3 z-30 select-none touch-none">
        ...
        {/* Report Button (when near dead body) */}
        <button
          type="button"
        ...
        <button
          ...
          disabled={!nearbyTask && !nearbyEmergencyButton}
